...
xvals = np.log(np.unique(df_cppe['nsites'].values))[4:]
yvals = np.log(df_cppe.query('runtype == "direct_scf"')['time'].values)[4:]
xvals = sm.add_constant(xvals)
model = sm.OLS(yvals, xvals)
results = model.fit()
print(results.summary())


yvals2 = np.log(df_cppe.query('runtype == "fmm_scf"')['time'].values)[4:]
model = sm.OLS(yvals2, xvals)
results = model.fit()
print(results.summary())



xvals = np.log(np.unique(df_pelib['sites'].values))[5:]
yvals = np.log(df_pelib.query('run == "direct_t_scf"')['time'].values)[5:]
xvals = sm.add_constant(xvals)
model = sm.OLS(yvals, xvals)
results = model.fit()
print(results.summary())
...
yvals2 = np.log(df_pelib.query('run == "fmm_t_scf"')['time'].values)[5:]
model = sm.OLS(yvals2, xvals)
results = model.fit()
print(results.summary())
...
# axes[0].set_xlim([1,2e5])
# axes[1].set_xlim([1,2e5])
# axes[2].set_xlim([1,2e5])
